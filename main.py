...
input_shape = 4  
output_shape = 2   
hidden_layer = [(nn.ReLU ,128),(nn.Tanh,128),(nn.Tanh,128),(nn.Tanh,128)]
dtype=torch.float32
...
trainer.add_loss(FHN_loos_fromDataSet(data_folder,1024*10,device=device,loss_type="L2",dtype=dtype),weigth=1)


#trainer.add_loss(FHN_loos_fromDataSet("training_data/treinocr/",1024*10,device=device,loss_type="L2",shuffle="True"),weigth=1)
#trainer.add_loss(FHN_loos_fromDataSet("training_data/treinocr2/",1024*10,device=device,loss_type="L2",shuffle="True",dtype=dtype),weigth=1)
...
def FHN_LOSS(data_in, model):
        ...
        pdeu =  10*(( 1)*(u*(u-0.4)*(1-u))-v +k*0.04 + 0.08)- dx_dt
        pdew =  10*(( 1)*(u*(u-0.4)*(1-u))-v +k*0.04 + 0.08)- dx_dt
...
trainer.add_validator(FHN_VAL_fromDataSet("training_data/treinocr2/",device=device,name="valcrit",dtype=dtype))
